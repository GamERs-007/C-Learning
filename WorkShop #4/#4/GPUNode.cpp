...
GPUNode::GPUNode(double m, long s, short n, long t, std::string col, Coordinate point)
:model(m),
speed(s),
node(n),
throughput(t),
...
void testPointer(){
	...
	//for loop by indexing
	//for(int i = 0, i < size, i++)

	//range-based for loop
	// int[] array;
	int * array;
	// array<int, x> //size is fixed //array takes consecutive memory spaces
	// for (Type& item: array)

	//C++ version 20+
	for (const GPUNode& node:std::span(ptr, 3)) {
		//node.setModel(node.getModel());
		std::cout << node.getModel() << std::endl;
	}
